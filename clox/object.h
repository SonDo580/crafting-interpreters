...
{
    Obj obj;
    Value *location;         // points to local variable's stack slot or 'closed'
    Value closed;            // where the closed-over variable lives on heap (after its stack slot is discarded)
    struct ObjUpvalue *next; // next upvalue in linked-list of all open upvalues
} ObjUpvalue;
...
{
    Obj obj;
    Value receiver; // only ObjInstance
    ObjClosure *method;
} ObjBoundMethod;
